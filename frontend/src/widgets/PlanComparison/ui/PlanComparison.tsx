...
const FALLBACK_PLANS: PlanResponse[] = [
  { slug: 'trial', name: 'Пробный', price: '0', period: '/ 10 дней', discount: 'Бесплатно', featured: false, features: [], tooltips: [] },
  { slug: 'basic', name: 'Базовый', price: '399', period: '/ месяц', discount: '−20% на первый платёж', featured: false, features: [], tooltips: [] },
  { slug: 'advanced', name: 'Продвинутый', price: '499', period: '/ месяц', discount: '−20% на первый платёж', featured: false, features: [], tooltips: [] },
];
...
const COMPARISON_ROWS: ComparisonRow[] = [
  { label: 'Цена', values: { trial: '0 ₽ / 10 дней', basic: '399 ₽ / месяц', advanced: '499 ₽ / месяц' }, bold: true },
  { label: 'Количество товаров', values: { trial: 'до 10', basic: 'до 100', advanced: 'безлимит' } },
  { label: 'Количество поставщиков', values: { trial: '2', basic: '15+', advanced: '50+' } },
  { label: 'Обновление цен', values: { trial: '24ч', basic: '6ч', advanced: 'real-time' } },
  { label: 'История цен', values: { trial: '1 месяц', basic: '6 месяцев', advanced: '12+ месяцев' } },
  { label: 'Экспорт отчётов (PDF/CSV)', values: { trial: '—', basic: '✓', advanced: '✓' } },
  { label: 'Нечёткий поиск (fuzzy)', values: { trial: '—', basic: '—', advanced: '✓' } },
  { label: 'API-доступ', values: { trial: '—', basic: '—', advanced: '✓' } },
  { label: 'Расширенная аналитика', values: { trial: '—', basic: '—', advanced: '✓' } },
  { label: 'Поддержка', values: { trial: '—', basic: '24/7', advanced: '24/7 + менеджер' } },
];
  ...
  return (
    <>
      <div className="text-center border-b-0 pt-0 max-w-[600px] mx-auto mb-12">
        ...
        {isLoading && <p className="mt-4 text-sm text-body-muted">Загрузка тарифов…</p>}
                ...
                return (
                  <th
                    ...
                    className={`font-montserrat text-[14px] font-medium uppercase tracking-[0.04em] text-center px-4 py-3 whitespace-nowrap ${
                      idx === 1 ? 'bg-slate text-ivory' : 'text-body-muted'
          ...
          <tbody>
            ...
            <tr>
              ...
              {SLUGS.map((slug, j) => {
                ...
                return (
                  <td key={slug} className={`px-4 py-5 text-center ${j === 1 ? 'bg-ivory-elevated' : ''}`}>
                    <Link href={href} className={btnClass}>{label}</Link>
                  </td>
                );
              })}
            </tr>
