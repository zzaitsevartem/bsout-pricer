...
export function planFeatures(plan: PlanResponse): string[] {
  const features = [
...
export const TariffPlans: React.FC<TariffPlansProps> = ({
  action = 'register',
  onSelect,
  selectedPlan,
  subscribing = false,
  ...
  return (
    <div>
        ...
        <div className="grid grid-cols-3 gap-6 items-start max-lg:grid-cols-1 max-lg:max-w-[480px] max-lg:mx-auto">
          {ordered.map((plan) => {
            ...
            return (
              ...
              >
                {isAdvanced && (
                  <div className="absolute -top-[8px] left-1/2 -translate-x-1/2 bg-green-discount text-white text-[13px] font-semibold px-4 py-[6px] rounded-full whitespace-nowrap shadow-md max-lg:static max-lg:translate-x-0 max-lg:mb-4 max-lg:rounded-full max-lg:text-center">
                    Выбирают чаще
                  </div>
                )}
                  ...
                  <div className="relative group">
                    <button
                      ...
                      className={`p-1 rounded-full transition-colors cursor-pointer ${
                        isAdvanced ? 'text-ivory/50 hover:text-ivory' : 'text-body-muted hover:text-body'
                      }`}
                ...
                <span
                  className={`block font-montserrat text-[11px] uppercase tracking-[0.08em] mb-4 pb-3 border-b ${
                    isAdvanced ? 'text-ivory/60 border-ivory/15' : 'text-body-muted border-border-light'
                  }`}
                >
                  {plan.nameRu}
                </span>
                ...
                {onSelect ? (
                  <button
                    ...
                    disabled={subscribing && selectedPlan === plan.plan}
                    className={`block w-full text-center rounded-full py-3 text-[15px] font-medium transition-colors cursor-pointer disabled:opacity-60 ${
                      isAdvanced
                  ...
                  >
                    {selectedPlan === plan.plan && subscribing
                      ? 'Создаём платёж…'
                      : plan.price === 0
                        ? 'Подключить'
                        : `Оплатить ${plan.nameRu}`}
                  </button>
                ) : (
                  <Link
                    href={href}
                    className={`block w-full text-center rounded-full py-3 text-[15px] font-medium no-underline transition-colors ${
                      isAdvanced
                        ? 'bg-ivory text-slate border border-ivory hover:bg-ivory-elevated'
                        : 'bg-transparent text-slate border border-slate hover:bg-ivory-elevated'
                    }`}
      ...
      {!plans.data && !plans.isLoading && !plans.isError && (
        <p className="text-center text-[15px] text-body-subtle">
          {trial
