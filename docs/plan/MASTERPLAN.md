...
| Поле | Значение |
|------|----------|
| Активный блок | — (ждёт решения владельца по порядку: рекомендация 09 → 10 → 11) |
...
| Готово (проверено) | **02** ядро данных, **05** fuzzy, **06** каркас парсеров, **03** тесты (бэкенд); **тесты: 161 зелёный** |
...
**Статус:** [ ] · **Заведён:** 2026-07-25 · **P0** · **Разблокирует 10, 11, 13** · Не требует сети
...
- [ ] CLI-точка входа для сида (`python -m src.cli seed`): справочники (`seed_catalog`) + 5 магазинов + категории. Идемпотентно.
- [ ] Расширить справочники до рабочего объёма (устройства Apple/Samsung/Xiaomi/Huawei/Honor + алиасы, типы деталей, классы качества, стоп-слова).
- [ ] Админский импорт офферов: `POST /api/admin/offers/import` (CSV/JSON) через существующий `ParserService.upsert_offer`.
- [ ] Фикстура-датасет (~200–500 офферов, 5 магазинов, пересекающиеся позиции) для демо и валидации матчинга.
- [ ] Сид как шаг деплоя (документировать в CI/compose).
