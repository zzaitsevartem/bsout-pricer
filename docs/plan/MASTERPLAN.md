...
### Фаза 3 — Расширение словаря устройств
- [ ] Массовое пополнение `devices` + `device_aliases` из реальных каталогов 5 магазинов: Samsung/Xiaomi/Honor/Realme/Tecno/POCO + алиасы по MPN/кодовому имени/A-номеру.
- [ ] Процесс пополнения: из unmatched-хвоста генерировать кандидатов-алиасов в очередь (петля дообучения из дизайна), а не хардкодить разово.
- [ ] Идемпотентный сид новых устройств/алиасов (не ломает существующий).

### Фаза 4 — Замер и приёмка (DoD)
- [ ] Повторный живой прогон по 5 магазинам → цель: **auto существенно выше 38%** (ориентир ~75–85% при заполненном словаре), candidate-band работает, false-merge ≈ 0 (ручной аудит выборки авто-связок).
- [ ] Результаты (до/после, по магазинам) → `MANUAL_VERIFICATION.md`.
