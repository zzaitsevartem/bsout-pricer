...
| Поле | Значение |
|------|----------|
| Активный блок | **11 — Фронт: поиск, карточка, сравнение, админка** (владелец: делать по порядку 09 → 10 → 11 → 12/13 → 14/15 → 07) |
...
| Готово (проверено) | **02** ядро данных, **05** fuzzy, **06** каркас парсеров, **03** тесты, **09** данные, **10** матчинг+сравнение; **тесты: 391**, сравнение цен между магазинами работает на 308 офферах |
...
**Статус:** [ ] · **Заведён:** 2026-07-25 · **P0** · **Зависит от 10** (частично можно начать после 09)
...
- [ ] **Сначала:** синхронизировать `models/product` (+ `search`, `admin`, `parser`) с реальным контрактом бэка; включить `safeParse` хотя бы в dev.
- [ ] `/search` на реальных данных: состояние в URL (`?q=&store=&category=&page=`), рабочие фильтры, пагинация, состояния loading/empty/error.
- [ ] Динамический маршрут `/product/[id]` на `useProduct` + `usePriceHistory`.
- [ ] UI сравнения: одна деталь — цены всех магазинов, подсветка минимума, переход в магазин.
- [ ] Админка на реальных данных (`useAdminStats/useAdminUsers/useToggleUserActive/useParsers/useRunParser`).
- [ ] Обработка платного гейта (403 → CTA на тарифы).
- [ ] `npm install && npm run build && npm run lint` (🚫 нужна сеть) + Vitest.
