...
- [ ] Модель + миграция `payments` (сумма, валюта, статус, provider_payment_id, идемпотентный ключ).
- [ ] Разорвать «нажал кнопку = получил подписку»: подписка активируется только по подтверждённой оплате.
- [ ] Подключить `require_active_subscription` к платным эндпоинтам; развести basic/advanced.
- [ ] Триал при регистрации + флаг «триал использован» на `User`.
- [ ] Матрица лимитов тарифов и её enforcement (см. Блок 13 — лимиты заданы через отслеживаемые товары).
...
- [ ] Отмена «до конца оплаченного периода» (сейчас доступ отбирается мгновенно, вопреки тексту в UI).
- [ ] `GET /api/payment/plans` — единый источник цен для фронта.
- [ ] 🚫 сеть: интеграция ЮKassa (создание платежа, редирект), вебхук с проверкой подписи и идемпотентностью, фискализация 54-ФЗ, возвраты.
