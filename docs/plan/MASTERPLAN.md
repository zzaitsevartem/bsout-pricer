...
| Поле | Значение |
|------|----------|
| Активный блок | **10 — Матчинг и сравнение цен** (владелец: делать по порядку 09 → 10 → 11 → 12/13 → 14/15 → 07) |
...
| Готово (проверено) | **02** ядро данных, **05** fuzzy, **06** каркас парсеров, **03** тесты, **09** данные/сид/импорт; **тесты: 211 зелёных**, в БД 308 офферов по 5 магазинам |
...
**Статус:** [ ] · **Заведён:** 2026-07-25 · **P0** · **Зависит от 09** · Не требует сети
...
- [ ] `catalog/service/normalizer.py` — нормализация под русские прайсы запчастей (сейчас `normalize_name` слишком примитивен, а он же кормит trgm-поиск).
- [ ] `catalog/service/extractor.py` — извлечение атрибутов: бренд, устройство (через `device_aliases`), тип детали (через синонимы), класс качества, цвет.
- [ ] Канонизация: `get_or_create` для `Product` (canonical_key) и `Cluster` (device+part_type); политика для неопознанных устройств (сейчас схема требует device_id — иначе оффер молча выпадает из сравнения).
- [ ] Связывание `store_offers.product_id` внутри upsert парсера (иначе матчер никогда не вызовется).
- [ ] Генерация `match_candidates` (pg_trgm + `similarity`) и автоматч по порогу; ниже порога — в модерацию.
- [ ] Пересчёт агрегатов кластера (`offers_count`, `min_price_retail`, `min_price_opt`).
- [ ] **API сравнения:** поиск с группировкой по каноническому товару; карточка «деталь → предложения всех магазинов + история цен»; честный «самый дешёвый».
- [ ] Админ-API модерации кандидатов (approve/reject).
- [ ] Деактивация исчезнувших офферов + реализация `full_sync` (сейчас параметр принимается, но не используется).
- [ ] Метрики качества матчинга (доля связанных офферов, средний score) + батчевый перематч.
