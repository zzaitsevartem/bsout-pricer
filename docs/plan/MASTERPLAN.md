...
**Статус:** [ ] · **Заведён:** 2026-07-25 · **P0** · **Зависит от 10**
...
- [ ] Модель `tracked_products` (пользователь ↔ канонический товар/кластер) + миграция.
- [ ] API: добавить/убрать/список отслеживаемого; лимит по тарифу (10/100/500) с понятной 402/403-ошибкой.
- [ ] Детект снижения цены при парсинге/импорте (по `offer_price_history` и агрегатам кластера).
- [ ] Уведомления для advanced: канал (email/телеграм — **решение владельца**), шаблоны, дедупликация, отписка.
- [ ] Фронт: «Отслеживать» в карточке и выдаче, раздел в кабинете, счётчик «использовано N/лимит» (сейчас на странице подписки это был фейковый «42/100»).
