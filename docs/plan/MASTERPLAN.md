...
- [x] 2026-07-26 Замер по всем магазинам (live, read-only extract): **siriust 64.7%**, **divizion 59.6%** (n=925), **liberti 41.3%** (n=1123), **tgsm 33.3%** (n=120); `unmatched`=0% везде (всё нераспознанное → `review`). Greenspark — вживую нельзя (нет парсера, бот-защита, block07). Разброс auto — словарь пополнялся под siriust-хвост; поднять остальные = прогнать `gaps` по их `review` и досеять устройства (та же петля).
