...
async def create_store(
    body: StoreCreateRequest, db: AsyncSession = Depends(get_db), admin=Depends(get_current_admin)
):
    existing = await StoreService.get_by_slug(db, body.slug)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Store with this slug already exists"
        )
    return await StoreService.create(
        db, name=body.name, slug=body.slug, website_url=body.website_url, logo_url=body.logo_url
    )
